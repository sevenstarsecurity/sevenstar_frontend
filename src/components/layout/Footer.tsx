"use client";

import React from "react";
import Link from "next/link";
import { ImageFallback } from "../ui/ImageFallback";
import { MapPin, Phone, Printer, Mail, ChevronRight } from "lucide-react";

export const Footer: React.FC = () => {
  return (
    <footer className="bg-[#091810] text-gray-300 pt-16 pb-8 border-t border-emerald-950">
      <div className="max-w-[1600px] mx-auto px-4 md:px-10 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b border-emerald-900/40">
          {/* Column 1: Brand Info (lg:col-span-4) */}
          <div className="lg:col-span-4 space-y-4">
            <div className="flex items-center">
              <ImageFallback
                src="/images/sevennobg.png"
                alt="Seven Star Security Logo"
                className="h-12 md:h-14 w-auto object-contain"
                fallbackText="sevenstarlogo.png"
              />
            </div>

            <p className="text-xs text-emerald-100/70 leading-relaxed max-w-sm">
              Seven Star Security Services is a premier security agency offering
              vetted personnel and electronic surveillance across Nepal since 2012.
            </p>

            {/* Social SVG Icons */}
            <div className="flex items-center space-x-3 pt-2">
              <a
                href="#facebook"
                aria-label="Facebook"
                className="w-8 h-8 rounded-full border border-emerald-700/60 bg-emerald-950/40 flex items-center justify-center text-emerald-400 hover:bg-[#0b4226] hover:text-white transition-all"
              >
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href="#twitter"
                aria-label="Twitter / X"
                className="w-8 h-8 rounded-full border border-emerald-700/60 bg-emerald-950/40 flex items-center justify-center text-emerald-400 hover:bg-[#0b4226] hover:text-white transition-all"
              >
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="#linkedin"
                aria-label="LinkedIn"
                className="w-8 h-8 rounded-full border border-emerald-700/60 bg-emerald-950/40 flex items-center justify-center text-emerald-400 hover:bg-[#0b4226] hover:text-white transition-all"
              >
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                </svg>
              </a>
              <a
                href="#instagram"
                aria-label="Instagram"
                className="w-8 h-8 rounded-full border border-emerald-700/60 bg-emerald-950/40 flex items-center justify-center text-emerald-400 hover:bg-[#0b4226] hover:text-white transition-all"
              >
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Column 2: Quick Links (lg:col-span-2) */}
          <div className="lg:col-span-2 space-y-4">
            <h4 className="text-xs font-extrabold text-white uppercase tracking-widest border-b border-emerald-800/40 pb-2">
              QUICK LINKS
            </h4>
            <ul className="space-y-2 text-xs">
              {[
                { name: "Home", href: "/" },
                { name: "About Us", href: "#about" },
                { name: "Services", href: "#services" },
                { name: "Careers", href: "#career" },
                { name: "Contact Us", href: "#contact" },
              ].map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="hover:text-emerald-300 transition-colors flex items-center gap-1 group"
                  >
                    <ChevronRight className="w-3 h-3 text-emerald-600 transition-transform group-hover:translate-x-0.5" />
                    <span>{link.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 3: Our Services (lg:col-span-3) */}
          <div className="lg:col-span-3 space-y-4">
            <h4 className="text-xs font-extrabold text-white uppercase tracking-widest border-b border-emerald-800/40 pb-2">
              OUR SERVICES
            </h4>
            <ul className="space-y-2 text-xs">
              {[
                "Security Guard Services",
                "VIP Protection",
                "CCTV & Surveillance",
                "Event Security",
                "Cash-In-Transit Management",
              ].map((service) => (
                <li key={service}>
                  <a
                    href="#services"
                    className="hover:text-emerald-300 transition-colors flex items-center gap-1 group"
                  >
                    <ChevronRight className="w-3 h-3 text-emerald-600 transition-transform group-hover:translate-x-0.5" />
                    <span>{service}</span>
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 4: Contact Us (lg:col-span-3) */}
          <div className="lg:col-span-3 space-y-4">
            <h4 className="text-xs font-extrabold text-white uppercase tracking-widest border-b border-emerald-800/40 pb-2">
              CONTACT US
            </h4>
            <ul className="space-y-3 text-xs text-emerald-100/80">
              <li className="flex items-start gap-2.5">
                <MapPin className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" />
                <span>
                  Kathmandu, Nepal (Regional Offices in Pokhara, Biratnagar)
                </span>
              </li>
              <li className="flex items-center gap-2.5">
                <Phone className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <span>+977 1 4531000</span>
              </li>
              <li className="flex items-center gap-2.5">
                <Printer className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <span>+977 1 4531001</span>
              </li>
              <li className="flex items-center gap-2.5">
                <Mail className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <span>info@sevenstar.com.np</span>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Copyright Bar */}
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] ">
          <p>© 2026 Seven Star Security Services. All Rights Reserved.</p>
          <div className="flex items-center gap-2 font-semibold tracking-wider uppercase">
            <span>GOVT LICENSE NO. 127991/2071/072</span>
          </div>
        </div>
      </div>
    </footer>
  );
};
